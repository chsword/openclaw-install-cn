...
1. 点击蓝色的「**安装 / 升级**」按钮
2. GUI 自动从 CDN 下载安装包并解压（见 [第 5 节 下载进度](#5-下载进度)）
3. 安装完成后显示绿色成功提示（见 [第 8 节](#8-安装成功)）
...
**原因：** 网络不通或 CDN 服务暂时不可用。

**解决方案：**
1. 确认当前网络环境能访问 `https://oclaw.chatu.plus`
2. 尝试在浏览器中打开 `https://oclaw.chatu.plus/manifest.json` 验证可达性
